...
### Baustellen
# Codebook-Checks
# weitere Checks? => Abgleich Ingest-Prüfung
# Excel Output?
# automatically fill in ID variable based on column position (1)
# Erklärungen, Beispiele, Verständlichkeit
# Lade-Screen

### tricky:
# Ergebnisse konditional "ok" vs. Output?

### offene Fragen:
# Deployment? Installation vs. Server-seitige Lösung? Hosting?
# siehe auch EDV
# vlt. alternativ auch Upload Server?
# parallele syntaktische Variante?
...
server <- function(input, output, session) {
  data <- reactive({
    req(input$upload)
    import_spss(input$upload$datapath)
  })
  ...
  output$encoding_names <- renderPrint({
    nam <- namesGADS(data())
    fixed_nam <- namesGADS(clean_encoding_data())
    nam[nam != fixed_nam]
  })
  
  output$encoding_labels <- renderPrint({
    all_differences <- equalGADS(data(), clean_encoding_data())
    all_differences$meta_data_differences
  })
  
  # id checks  
  id_vec <- reactive({
    suppressMessages(id_gads <- extractVars(data(), vars = input$id))
    extractData2(id_gads, convertMiss = TRUE)[[1]]
  })
  ...
  output$missing_varLabels <- renderPrint({
    varLabels <- unique(extractMeta(data())[, c("varName", "varLabel")])
    varLabels[is.na(varLabels$varLabel), "varName"]
  })
  
  # missing value labels
  output$missing_labels <- renderPrint({
    out <- checkMissingValLabels(data())
  ...
  output$missing_range <- renderPrint({
    ...
    suppressMessages(checked_dat <- checkMissingsByValues(data(), missingValues = missing_range))
    changed_vars <- equalGADS(data(), checked_dat)$meta_data_differences
    changed_meta <- extractMeta(data(), changed_vars)
    changed_meta[changed_meta$value %in% missing_range, ]
  })
  
  output$missing_regex <- renderPrint({
    suppressMessages(checked_dat <- checkMissings(data(), missingLabel = input$missing_regex))
    changed_vars <- equalGADS(data(), checked_dat)$meta_data_differences
    changed_meta <- extractMeta(data(), changed_vars)
  ...
  output$sdc <- renderPrint({
    ...
    exclude_vars <- setdiff(namesGADS(data()), sdc_vars)
    out <- sdc_check(input$upload$datapath, exclude = exclude_vars)
